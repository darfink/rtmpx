...
pub enum RtmpMessage {
    ...
    /// A command on an AMF3-capable message stream (message type 17).
    ///
    /// Mirrors `Amf0Command`: `command_name` is a string, `transaction_id` is
    /// numeric, and `command_object` carries the `connect`/`createStream`
    /// properties. Values are always presented as [`Amf3Value`] regardless of
    /// how they were framed, so callers do not have to branch; `format` records
    /// the framing so a relay can reproduce it.
    Amf3Command {
        command_name: String,
        transaction_id: f64,
        command_object: Amf3Value,
        additional_arguments: Vec<Amf3Value>,
        /// The format selector this message used. [`AmfEncoding::Amf0`] means
        /// the values were AMF0 encoded behind a `0x00` selector, which is what
        /// Flash, librtmp and FFmpeg emit; [`AmfEncoding::Amf3`] means a `0x03`
        /// selector and a genuine AMF3 body.
        format: AmfEncoding,
    },

    /// A data message on an AMF3-capable message stream (type 15). See
    /// [`RtmpMessage::Amf3Command`] for how `format` is used.
    Amf3Data {
        values: Vec<Amf3Value>,
        format: AmfEncoding,
    },

    /// Shared-object message encoded as AMF0 (type 19). Carried opaquely for relay.
    Amf0SharedObject { data: Bytes },

    /// Shared-object message encoded as AMF3 (type 16). Carried opaquely for relay.
    Amf3SharedObject { data: Bytes },
...
impl RtmpMessage {
    ...
    pub fn get_message_type_id(&self) -> u8 {
        match *self {
            ...
            RtmpMessage::Amf0SharedObject { data: _ } => 19_u8,
            RtmpMessage::Amf3SharedObject { data: _ } => 16_u8,
